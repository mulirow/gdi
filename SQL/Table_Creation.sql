...
CREATE TABLE FazParte(
    ...
    CONSTRAINT FK_FAZ_PARTIDA FOREIGN KEY (ID_PARTIDA) REFERENCES Partida(ID_PARTIDA),
...
CREATE TABLE Possuem(
    ...
    CONSTRAINT PK_POSSUEM PRIMARY KEY (RIOT_ID, ID_PARTIDA)
);

CREATE TABLE Ter(
    NOME_AGENTE VARCHAR(30),
    ID_SKIN NUMBER(3, 0),
    CONSTRAINT FK_TER_AGENTE FOREIGN KEY (NOME_AGENTE) REFERENCES Agente(NOME_AGENTE),
    CONSTRAINT FK_TER_SKIN FOREIGN KEY (ID_SKIN) REFERENCES Skin(ID_SKIN),
    CONSTRAINT PK_TER PRIMARY KEY (NOME_AGENTE, ID_SKIN)
);
