...
SELECT J.RIOT_ID AS NOME_DO_OSTENTADOR, COUNT(P.ID_SKIN) AS QTD_SKINS, SUM(S.PRECO) AS VP_GASTOS
FROM (Jogador J LEFT OUTER JOIN Possuem P ON J.RIOT_ID = P.RIOT_ID) LEFT OUTER JOIN Skin S ON P.ID_SKIN = S.ID_SKIN
WHERE NOT EXISTS (
    SELECT *
    FROM Participa PA
    WHERE PA.RIOT_ID = J.RIOT_ID
) AND EXISTS (
    SELECT *
    FROM Amizade A
    WHERE (A.ADICIONA = J.RIOT_ID) OR (A.ADICIONADO = J.RIOT_ID)
)
GROUP BY J.RIOT_ID